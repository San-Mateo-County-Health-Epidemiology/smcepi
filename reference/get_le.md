...
This function is meant to be used on the output from the
[`make_life_table()`](https://san-mateo-county-health-epidemiology.github.io/smcepi/reference/make_life_table.md)
function. It will pull the life expectancy for a given age from the life
table.
...
- start_age:
  ...
  an integer for the start age of the age group for which you want life
  expectancy. Defaults to 0. Possible start ages are 0, 1 and multiples
  of 5 up to 90 (ie 5, 10, 15, 20...)

- grouping_vars:

  the same list of `grouping_vars` specified in the
  [`make_life_table()`](https://san-mateo-county-health-epidemiology.github.io/smcepi/reference/make_life_table.md)
  function

- include_ci:

  option to include the columns with the upper and lower 95% confidence
  interval for the estimate. Defaults to TRUE

## Value

a data frame with life expectancy for specified ages for each specified
group
